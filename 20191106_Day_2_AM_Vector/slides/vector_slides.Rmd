...
.Large[
- Using the `starwars` data set and `dplyr` syntax find the average mass of humans grouped by homeworld ordered by average_mass column.
]
---
...
- Using `ggplot2` plot only *two* layers 
  - `big_fires` 
  - `fire_districts` <- only "Stuart Nechako", "Quesnel", "Cariboo-Chilcotin", and "Prince George" natural resource districts (`nr_district`)
]
---
## Solution
...
```{r}
p <- ggplot() +
  geom_sf(data = quesnel_district, fill = "grey80", alpha = 0.5) +
...
p + geom_sf(data = unionized, size = 1.5, fill = NA, colour = "slateblue")
```
...
p + geom_sf(data = intersected, size = 1.5, fill = NA, colour = "slateblue")
```
---
## `st_difference`
```{r}
differenced <- st_difference(quesnel_district, biggest_fire)
p + geom_sf(data = differenced, size = 1.5, fill = NA, colour = "slateblue")
```
...
## Geometry Predicates
TODO: map to ArcGIS names
These are just subsetting
