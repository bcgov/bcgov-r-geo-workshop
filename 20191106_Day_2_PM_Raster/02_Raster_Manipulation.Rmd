...
In this section we will work through a typical workflow to preparing multiple rasters, manipulating them to get an common raster alignment (stack). We will also explore using vector data as a boundary and to extract values from a stacked raster at XY points. 
...
An advantage of rasters is that we can stack them together to perform functions on mulitple layers at once. We can use `RasterStacks` or `RasterBricks`. Both of these can handle multiple layers. It is also important to note other packages are developing methods for storing large raster datasets such as `gdalcubes`. 

Raster Bricks typically correspond to single multispectral satellite image. We will use these when dealing with remote sensed datasets. 
...
```{r optional local drive,  eval = FALSE }
...
We will read in the dem `raster` and generate slope and aspect using the terrain function. (See details in [Bonus Section](https://github.com/bcgov/bcgov-r-geo-workshop/blob/master/20191106_Day_2_PM_Raster/01_Raster_Basics.Rmd). 
...
2) Using the `cellStats()` function, calculate the mean and sd of each raster in the stack. Hint : check the required parameters by using `?cellStats`.
...
In practice, we normall obtain raster data from multiple sources and it is common to preprocess rasters to the same extents and cell size before they can be used in an analysis. 

Lets read in a slope layer and build a stack with our `dem` raster. Firstly we need to compare the resolution, extents and crs for each raster. 
...
Now check the extents and if they match try to `stack` the rasters. 
...
2) Compare the resolution, extent, crs with the dem raster and predict if stack will be suceesful/ not succesful and why? 


</div>

```{r dissagregation part1, eval = TRUE, echo = FALSE}
...
In this example we will use our raster extent to create an aoi. We will then extract the BEC zone data from `bcmaps` package and extract the SBSmc2 area within our raster extent. 

Firstly we will create an area of interest using out raster extent. 


```{r load libraries, echo = T, results = "hide", warning=FALSE}
library(sf)
library(dplyr)
library(bcmaps)
...
# We are only interested in the SBSmc2 type : lets assign a number 
sbs <- bec_aoi %>%
...
```{r plots dem and clip, echo = TRUE, eval = FALSE}
plot(dem)
...
This function created a new raster that has the same values as a defined raster, except where the cells are NA. These cells will become `NA`'s. 
