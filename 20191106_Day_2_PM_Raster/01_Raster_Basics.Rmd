...
This document outlines some basic operations for rasters. You can follow along by running the code (within markdown or as an R file) or refer to the html version. 
...
Coordinate Reference Systems come in three formats (`ESPG`, `proj4string`, `WKT`). Raster data commonly relies on `proj4string` type. In this case the `+` seperates each element. You can think of this as a common seperating each value in a csv file. Tip: Mapping with leaflet and other web-mapping requires WGS84 (`EPSG: 4326`). We can see the crs when we call the raster by name or directly using the `crs()` command.
